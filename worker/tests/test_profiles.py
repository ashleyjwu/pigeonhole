...
from pigeonhole_worker.profiles import TrackFacts, build_profile
...
def test_genre_dist_normalizes_to_top_tag() -> None:
    tracks: list[TrackFacts] = [
        (["a1"], 2020, None, [{"dream pop": 1.0, "shoegaze": 0.5}]),
        (["a2"], 2020, None, [{"dream pop": 1.0}]),
    ]
    profile = build_profile("p", tracks)
    # dream pop: 1.0 + 1.0 = 2.0 (top); shoegaze: 0.5
    assert profile.genre_dist == {"dream pop": 1.0, "shoegaze": 0.25}


def test_genre_dist_merges_multiple_artists_per_track_by_max_not_sum() -> None:
    # A single track with two artists who share a tag: should NOT double-count
    # within one track (max, not sum), unlike across different tracks.
    tracks: list[TrackFacts] = [(["a1", "a2"], 2020, None, [{"indie": 1.0}, {"indie": 0.6}])]
    profile = build_profile("p", tracks)
    assert profile.genre_dist == {"indie": 1.0}


def test_genre_dist_empty_when_no_track_has_tags() -> None:
    profile = build_profile("p", [(["a1"], 2020, None, [])])
    assert profile.genre_dist == {}


def test_genre_dist_ignores_tracks_with_no_tags_but_uses_ones_that_have_them() -> None:
    tracks: list[TrackFacts] = [
        (["a1"], 2020, None, []),  # no tag data for this track
        (["a2"], 2020, None, [{"indie": 1.0}]),
    ]
    profile = build_profile("p", tracks)
    assert profile.genre_dist == {"indie": 1.0}
